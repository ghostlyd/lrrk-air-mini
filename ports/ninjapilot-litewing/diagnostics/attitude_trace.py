...
OBJECT_ID=0x5E93B7FE
WIRE=struct.Struct('<4I13f8H3H6B')

def decode(payload):
    if len(payload)!=WIRE.size: raise ValueError('trace payload must be 96 bytes')
    v=WIRE.unpack(payload)
    count,trigger,index,version,state,available,known,suppression,reserved=v[25:]
    if (version!=1 or state not in range(4) or available not in (0,1) or
    ...
    if state==3:
        ...
        if any(payload[:84]) or available or known or suppression:
    ...
    requested=list(v[17:21]);submitted=list(v[21:25])
    ...
    return dict(timestamp_us=v[0]+(v[1]<<32),sequence=v[2],pwm_commits=v[3],
        ...
        rpy=list(v[14:17]),requested=requested,submitted=submitted,count=count,
