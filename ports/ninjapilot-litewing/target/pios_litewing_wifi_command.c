...
static int healthy_random(void *ctx, uint8_t *out, size_t size)
{
    ...
    if (!s->healthy_ap || lw_wifi_ap_faults()) return -1;
    esp_fill_random(out, size);
    if (lw_wifi_ap_faults()) {
...
static int ap_address(esp_netif_ip_info_t *info)
{
    ...
    for (;;) {
        ...
        if (now < previous || now - started >= READY_TIMEOUT_US || lw_wifi_ap_faults()) return -1;
        previous = now;
        if (esp_netif_is_netif_up(netif)) {
            if (esp_netif_get_ip_info(netif, info) != ESP_OK) return -1;
...
{
    if (!s->have_peer || !written || written > sizeof(s->reply) || lw_wifi_ap_faults()) return -1;
...
static int service(struct command_state *s, int socket_fd)
{
    ...
    if (c->owned && c->session.phase == LW_CLOSED) return -1;
    if (s->have_peer) {
        size_t written = 0;
...
static void command_task(void *arg)
{
    ...
    int socket_fd = -1;
    if (lw_wifi_ap_start(s->root) != 0 || lw_wifi_ap_faults()) goto end;
    esp_netif_ip_info_t ip = {0};
    if (ap_address(&ip) != 0 || lw_wifi_ap_faults()) goto end;
    s->healthy_ap = true;
    socket_fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (socket_fd < 0) goto end;
    int flags = fcntl(socket_fd, F_GETFL, 0);
    if (flags < 0 || fcntl(socket_fd, F_SETFL, flags | O_NONBLOCK) < 0) goto end;
    struct sockaddr_in local = {.sin_family = AF_INET,
        .sin_port = htons(LW_WIFI_COMMAND_PORT), .sin_addr.s_addr = ip.ip.addr};
    if (bind(socket_fd, (struct sockaddr *)&local, sizeof(local)) < 0) goto end;
    ...
    if (getsockname(socket_fd, (struct sockaddr *)&actual, &actual_size) < 0 ||
        ...
        !same_peer(&local, &actual)) goto end;
    ...
    for (;;) {
        ...
        for (unsigned count = 0; count < READS_PER_TURN; ++count) {
            int64_t now = esp_timer_get_time();
            if (now < turn_start || lw_wifi_ap_faults()) goto end;
            ...
            if (result == LW_RETIRED || lw_wifi_ap_faults()) goto end;
...
{
    bool expected = false;
    if (!atomic_compare_exchange_strong(&launched, &expected, true)) return -1;
    if (xTaskCreate(command_task, "lw-command", STACK_BYTES, NULL,
                    tskIDLE_PRIORITY + 1, NULL) != pdPASS) {
        atomic_store(&launched, false);
        return -1;
    }
    return 0;
}
