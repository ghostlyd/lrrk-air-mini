...
uint16_t UAVObjGetNumBytes(UAVObjHandle h) { (void)h; return 96; }
...
UAVObjHandle LiteWingAttitudeTraceHandle(void) { return (void *)(uintptr_t)0x5E93B7FE; }
...
int main(void)
{
    ...
    assert(wire[90] == 1 && wire[91] == 1 && wire[84] == 0);
    ...
    for (unsigned i=0; i<1448; ++i) {
        ...
        LiteWingAttitudeTraceRecord(.002f,a,g,c,r);
    }
    ...
    uint8_t saved[96]; memcpy(saved,wire,96);
    LiteWingAttitudeTraceRecord(.5f,a,g,c,r);
    assert(LiteWingAttitudeTracePack(obj, 511, wire) == 0 && !memcmp(saved,wire,96));
