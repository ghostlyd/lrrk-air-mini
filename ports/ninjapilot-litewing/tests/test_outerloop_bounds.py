...
class OuterloopBoundsTests(unittest.TestCase):
    ...
    def test_checked_startup_rejects_each_object_scheduler_and_callback_failure(self):
        for case in ([f"startup-object-{index}" for index in range(1, 7)] +
                     ["startup-scheduler", "startup-callback"]):
            with self.subTest(case=case): self.run_case(case)

    def test_checked_startup_accepts_existing_objects_and_is_one_shot(self):
        self.run_case("startup-existing")
        self.run_case("startup-repeat")

    def test_checked_startup_mutations_are_detected(self):
        code = self.source.read_text()
        mutations = {
            "object-result": ("if (!RateDesiredHandle() && (RateDesiredInitialize() != 0 || !RateDesiredHandle())) return -1;",
                              "(void)RateDesiredInitialize();", "startup-object-1"),
            "scheduler-result": ("if (!callbackHandle) return -1;", "if (!callbackHandle && false) return -1;", "startup-scheduler"),
            "callback-result": ("if (AttitudeStateConnectCallback(AttitudeStateUpdatedCb) != 0) return -1;",
                                "if (AttitudeStateConnectCallback(AttitudeStateUpdatedCb) != 0 && false) return -1;", "startup-callback"),
            "one-shot": ("if (outerloopInitAttempted) return -1;", "if (outerloopInitAttempted && false) return -1;", "startup-repeat"),
        }
        for name, (old, new, case) in mutations.items():
            with self.subTest(mutant=name):
                self.assertEqual(code.count(old), 1, name)
                source = self.output / ("startup-" + name + ".c")
                source.write_text(code.replace(old, new))
                binary = self.compile(source, "startup-" + name)
    ...
    def test_real_sdk_selects_all_generated_stabilization_sources_and_watches_inputs(self):
        build = os.environ.get("LRRK_IDF_BUILD_DIR")
        if not build: self.skipTest("real IDF graph not supplied")
        build = Path(build).resolve()
        commands = json.loads((build / "compile_commands.json").read_text())
        for name in ("stabilization.c", "outerloop.c", "innerloop.c"):
            with self.subTest(source=name):
                entries = [e for e in commands if Path(e["file"]).name == name]
                self.assertEqual(len(entries), 1)
                selected = Path(entries[0]["file"]).resolve()
                self.assertEqual(selected, build / "esp-idf/main/litewing_stabilization" / name)
                self.assertEqual(selected.read_bytes(), (self.output / name).read_bytes())
        ...
        for path in (ROOT / "prepare_stabilization.py", ROOT / "prepare_startup.py",
                     *(self.flight / "flight/modules/Stabilization" / name
                       for name in ("stabilization.c", "outerloop.c", "innerloop.c"))):
