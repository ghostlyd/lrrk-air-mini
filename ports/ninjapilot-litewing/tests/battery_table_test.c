...
static bool health_registered, pwm_registered;
...
int32_t LiteWingPwmObservationInitialize(void) {
    assert(inits==5 && health_registered);
    if (!strcmp(scenario,"pwm-init-fail")) return -8;
    pwm_registered=true; return 0;
}
int32_t LiteWingImuHealthStart(void) { assert(starts==7); return 0; }
#define MODULE(Name, Index) \
int32_t Name##Initialize(void) { assert(inits++==Index); if(Index==5) assert(health_registered && pwm_registered); return 0; } \
...
int main(int argc,char **argv) {
    ...
    if (!strcmp(scenario,"pwm-init-fail")) {
        assert(rc==-8 && inits==5 && battery_inits==0);
        assert(PIOS_LiteWing_ModulesInitialize()!=0);
        assert(PIOS_LiteWing_ModulesStart()!=0 && starts==0);
        puts("PASS"); return 0;
    }
